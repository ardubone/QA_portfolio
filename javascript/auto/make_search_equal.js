...
async function testYaRu(){
    ...
    await page.waitForNavigation(); // команда ожидания загрузки результата

    console.log('Получение элементов результата поиска');
    const result = await page.$('.serp-item'); // создание переменной result и положить в неё элемент поисковой выдачи
